...
# Let's also see how many loggers we have
logger_clean %>%
  group_by(Logger_ID) %>%
...
## Define High Tide Periods

We'll consider a reading to be during "high tide" if it falls within a window around the high tide time. A typical approach is to use ±2 hours around each high tide.
```{r}
...
write_csv(logger_summary, "data/logger_summary_statistics.csv")

cat("Data exported successfully!\n")
cat("- merged_tide_temperature_data.csv\n")
cat("- logger_summary_statistics.csv\n")
```
